    ...
    public function actionAddNodes(): Response
    {
        ...
        foreach ($nodesPost as $key => $nodePost) {
            ...
            if (!Craft::$app->getElements()->saveElement($node, true)) {
                return $this->asModelFailure($node, Craft::t('menu-manager', 'Couldn’t add node.'), 'node');
            }
        }

        return $this->asSuccess(Craft::t('menu-manager', 'Node{plural} added.', ['plural' => count($nodesPost) > 1 ? 's' : '']));
    }
