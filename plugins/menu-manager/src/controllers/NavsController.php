...
use mycompany\menumanager\models\Nav_SiteSettings;
...
class NavsController extends Controller
{
    ...
    public function actionIndex(): Response
    {
        ...
        $navigations = MenuManager::$plugin->getNavs()->getEditableNavsForSite($site);

        $editable = $settings->bypassProjectConfig || Craft::$app->getConfig()->getGeneral()->allowAdminChanges;

        return $this->renderTemplate('menu-manager/navs/index', [
    ...
    public function actionEditNav(int $navId = null, Nav $nav = null): Response
    {
        if ($nav === null) {
            if ($navId !== null) {
                $nav = MenuManager::$plugin->getNavs()->getNavById($navId);
        ...
        return $this->renderTemplate('menu-manager/navs/_edit', [
            'navId' => $navId,
            'nav' => $nav,
        ]);
    }
    ...
    public function actionBuildNav(int $navId = null): Response
    {
        ...
        $nodes = MenuManager::$plugin->getNodes()->getNodesForNav($nav->id, $site->id);

        $parentOptions = MenuManager::$plugin->getNodes()->getParentOptions($nodes, $nav);

        Craft::$app->getSession()->authorize('editStructure:' . $nav->structureId);

        $editable = $settings->bypassProjectConfig || Craft::$app->getConfig()->getGeneral()->allowAdminChanges;

        return $this->renderTemplate('menu-manager/navs/_build', [
            'navId' => $navId,
            'nav' => $nav,
            'nodes' => $nodes,
            'site' => $site,
    ...
    public function actionSaveNav(): ?Response
    {
        ...
        if (!MenuManager::$plugin->getNavs()->saveNav($nav)) {
            $this->setFailFlash(Craft::t('menu-manager', 'Unable to save navigation.'));
        ...
        $this->setSuccessFlash(Craft::t('menu-manager', 'Navigation saved.'));
    ...
    public function actionReorderNav(): Response
    {
        ...
        MenuManager::$plugin->getNavs()->reorderNavs($navIds);
    ...
    public function actionDeleteNav(): Response
    {
        ...
        MenuManager::$plugin->getNavs()->deleteNavById($navId);
    ...
    public function actionDuplicateNav(): ?Response
    {
        ...
        if (!MenuManager::$plugin->getNavs()->saveNav($newNav)) {
            $this->setFailFlash(Craft::t('menu-manager', 'Unable to duplicate navigation.'));
