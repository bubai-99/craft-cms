<?php
namespace mycompany\menumanager\elements;

use mycompany\menumanager\MenuManager;
use mycompany\menumanager\elements\conditions\NodeCondition;
use mycompany\menumanager\elements\db\NodeQuery;
use mycompany\menumanager\events\NodeActiveEvent;
...
use mycompany\menumanager\nodetypes\SiteType;
use mycompany\menumanager\records\Node as NodeRecord;
...
class Node extends Element
{
    ...
    {
        return Craft::t('menu-manager', 'Node');
    }

    public static function pluralDisplayName(): string
    {
        return Craft::t('menu-manager', 'Nodes');
    }
    ...
    protected static function defineSources(string $context): array
    {
        ...
        $navs = MenuManager::$plugin->getNavs()->getEditableNavs();
    ...
    protected static function defineFieldLayouts(?string $source): array
    {
        if ($source === null || $source === '*') {
            ...
            if (preg_match('/^nav:(.+)$/', $source, $matches)) {
                $nav = MenuManager::$plugin->getNavs()->getNavByUid($matches[1]);
    ...
    protected static function defineActions(string $source): array
    {
        ...
        if (preg_match('/^nav:(\d+)$/', $source, $matches)) {
            $nav = MenuManager::$plugin->getNavs()->getNavById($matches[1]);
        ...
        Event::on(Structures::class, Structures::EVENT_BEFORE_MOVE_ELEMENT, function(MoveElementEvent $event) {
            ...
            if (property_exists($event, 'targetElementId')) {
                if ($nav->maxNodesSettings && $node = $event->getTargetElement()) {
                    MenuManager::$plugin->getNodes()->setTempNodes([$node]);
    ...
    public function getChipLabelHtml(): string
    {
        ...
        return parent::getChipLabelHtml() . ($html ? Html::tag('span', $html, ['class' => 'node-info-icons']) : '') . Html::tag('a', Craft::t('menu-manager', 'Edit'), ['class' => 'btn small icon edit node-edit-btn']);
    }
    ...
    public function getNav(): Nav
    {
        ...
        $nav = MenuManager::$plugin->getNavs()->getNavById($this->navId);
    ...
    {
        $registeredElements = MenuManager::$plugin->getElements()->getRegisteredElements(false);
    ...
    public function beforeSave(bool $isNew): bool
    {
        ...
        if (!$this->duplicateOf && $this->hasNewParent()) {
            if ($parentId = $this->getParentId()) {
                $parentNode = MenuManager::$plugin->getNodes()->getNodeById($parentId, '*', [
            ...
            function($attribute, $params, Validator $validator): void {
                ...
                if ($nav->maxNodes) {
                    if ($nav->isOverMaxNodes($this)) {
                        $validator->addError($this, $attribute, Craft::t('menu-manager', 'Exceeded maximum allowed nodes ({number}) for this nav.', ['number' => $nav->maxNodes]));
                    }
                }
                ...
                if ($nav->maxNodesSettings) {
                    ...
                    if ($nav->isOverMaxLevel($this)) {
                        $validator->addError($this, $attribute, Craft::t('menu-manager', 'Exceeded maximum allowed nodes for this level.'));
                    }
                }
            },
        ...
        $rules[] = [
            ...
            function($attribute, $params, Validator $validator): void {
                ...
                if (!$this->getIsDraft() && $this->isElement() && empty($this->elementId)) {
                    ...
                    $validator->addError($this, 'elementId', Craft::t('menu-manager', 'Element ID is required.'));
                    $validator->addError($this, 'linkedElementId', Craft::t('menu-manager', 'Linked Element ID is required.'));
                }
            },
            'skipOnEmpty' => false,
            'on' => [self::SCENARIO_DEFAULT, self::SCENARIO_LIVE, self::SCENARIO_ESSENTIALS],
    ...
    protected function metaFieldsHtml(bool $static): string
    {
        ...
        $fields[] = (function() use ($static) {
            ...
            foreach (MenuManager::$plugin->getNavs()->getBuilderTabs($this->getNav()) as $tab) {
                $nodeTypeOptions[] = [
...
EOD;
            ...
            return Cp::selectFieldHtml([
                'label' => Craft::t('menu-manager', 'Type'),
                'id' => 'type',
                'name' => 'type',
                'value' => $this->type,
        ...
        $fields[] = (function() use ($static) {
            if ($parentId = $this->getParentId()) {
                $parent = MenuManager::$plugin->getNodes()->getNodeById($parentId, $this->siteId);
            } else {
